...
import com.dean.R;
import com.dean.log.utils.LogCollectorUtility;
import com.dean.log.utils.LogHelper;
...
public class LogFileStorage
{
    ...
    {
        File dir = mContext.getFilesDir();
        File logFile = new File(dir, LogCollectorUtility.getMid(mContext) + LOG_SUFFIX);
    ...
    public boolean saveLogFile2SDcard(String logString, boolean isAppend)
    {
        ...
        try
        {
            File logDir = getExternalLogDir(mContext.getResources().getString(R.string.app_name));
            if (!logDir.exists())
            {
                logDir.mkdirs();
            }

            File logFile = new File(logDir, LogCollectorUtility.getMid(mContext) + LOG_SUFFIX);
            /*if (!isAppend) {
    ...
    private File getExternalLogDir(String path)
    {
        File logDir = LogCollectorUtility.getExternalDir(mContext, path, "Log");
